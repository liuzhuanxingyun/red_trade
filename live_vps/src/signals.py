...
def ema_atr_filter(exchange, symbol, ema_period, atr_period, multiplier, atr_threshold_pct, forbidden_hours=None):
    ...
    try:
        ...
        for i, (ts, vol) in enumerate(zip(df.index[-2:], df['volume'].iloc[-2:]), 1):
        ...
        last_close = df['close'].iloc[-1]  # 上一根k线的收盘价
        prev_close = df['close'].iloc[-2]  # 上上根k线的收盘价
        last_upper_band = df['upper_band'].iloc[-1]  # 上一根k线的上轨
        last_lower_band = df['lower_band'].iloc[-1]  # 上一根k线的下轨
        prev_upper_band = df['upper_band'].iloc[-2]  # 上上根k线的上轨
        prev_lower_band = df['lower_band'].iloc[-2]  # 上上根k线的下轨
        ...
        last_color = df['close'].iloc[-1] > df['open'].iloc[-1]  # True: 绿（上涨），False: 红（下跌）
        prev_color = df['close'].iloc[-2] > df['open'].iloc[-2]
