...
class EmaAtrStrategy(Strategy):
    ...
    def next(self):
        ...
        # 计算止损和止盈距离
        sl_distance = self.atr * self.sl_multiplier  # 使用止损ATR乘数计算止损距离
...
if is_batch_test:
    ...
    rr_range = [1]
    sl_multiplier_range = [1]  # 止损ATR乘数范围，示例固定为1，可修改为范围如 range(1, 5)
    
    # 自动计算组合总数
    total_combinations = (len(ema_period_range) * len(atr_period_range) * 
                          len(multiplier_range) * len(atr_threshold_pct_range) * len(rr_range) * len(sl_multiplier_range))
    ...
    stats, heatmap= bt.optimize(
        ...
        sl_multiplier=sl_multiplier_range,  # 新增止损ATR乘数优化参数
    ...
    heatmap_df.columns = ['ema_period', 'atr_period', 'multiplier', 'atr_threshold_pct', 'rr', 'sl_multiplier', 'win_rate']
